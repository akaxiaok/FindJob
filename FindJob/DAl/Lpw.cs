...
namespace FindJob.DAl
{
    public class Lpw : IZhaoPin
    {
        ...
        public void GetJobListFromWeb()
        {
            ...
            HtmlDocument htmlDoc = htmlWeb.Load(string.Format("http://www.liepin.com/zhaopin/?key={0}&dqs={1}&curPage={2}", _pars.Key, DataClass.GetDic_liepin(_pars.Addr), _pars.Page));

            var nodeList = htmlDoc.DocumentNode.SelectNodes("//ul[@class='sojob-result-list']/li ").AsParallel().ToList();
